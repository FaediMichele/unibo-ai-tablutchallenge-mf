...
class Model:
    ...
    def __init__(self, path: str='simple_reinforce_no_repetition', model_path:str=None) -> None:
    ...
    def loss_fn(self, states, z, v_pred, p_pred, actions_tensor):
        ...
        p_pred_flat = tf.reshape(p_pred, [p_pred.shape[0], 7 * 7 * 14])
        p_pred_flat = tf.nn.log_softmax(p_pred_flat, axis=1)
        p_softmax = tf.reshape(p_pred_flat, [p_pred.shape[0], 7, 7, 14])
    ...
    def create_model(self) -> keras.Model:
        ...
        # Extract the policy
        policy = layers.Conv2D(14, 1, 1, 'same', activation='linear')(x)
        # Define the model
