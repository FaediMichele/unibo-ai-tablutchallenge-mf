...
from .util import boolean_mask_from_coordinates, no_repetition_actions, policy_to_policy_matrix, actions_to_indexes, SAVE_ITERATIONS, BOARD_SIZE

class Model:
    # TODO do a reg net instead
    def __init__(self, path: str=None, remote=False, server_url="http://0.0.0.0:6000") -> None:
        if path is None:
            path = f'models/{"" if BOARD_SIZE == 9 else "simple_"}alpha_reinforce'
    ...
    @tf.function
    def loss_reinforce(self, states, z, v_pred, p_pred, actions_tensor):
        ...
        return state_value_loss + policy_loss + repeated_actions, policy_loss, state_value_loss
    ...
    def train_batch(self,
        ...
        with tf.GradientTape() as tape:
            ...
            loss, _, _ = self.loss_reinforce(batch_state,
                                                batch_z,
                                                v_pred,
                                                p_pred,
                                                batch_action_taken)
    ...
    def get_last_samples(self, number_of_samples: int=100_000
        ...
        for filename, _ in sorted(file_indexes, key=lambda x: x[1],
                                  reverse=True):
            file_data = self.sync_file_op(os.path.join(self.path, filename), "rb", lambda f: pickle.load(f))
