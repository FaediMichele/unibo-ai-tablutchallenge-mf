...
import pickle
import datetime
from games.tictactoe.game import State, Action
import requests
import fcntl
import shutil
...
class Model:
    ...
    def __init__(self, path: str=None, old_model: bool=False, remote=False, server_url="http://0.0.0.0:6000") -> None:
        ...
        '''
        if path is None:
            path = f'models/{"" if BOARD_SIZE == 9 else "simple_"}alpha_zero_tictactoe'
    ...
    def train_episode(self, states: list[tuple[tf.Tensor,
        ...
        policy_matrix = tf.reshape(policy_matrix, (-1, BOARD_SIZE, BOARD_SIZE))
