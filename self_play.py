...
def get_user_inputs():
    ...
    parser.add_argument("--player_class", choices=["r", "z"], help="Player 1 class (Reinforce or AlphaZero)")
...
def main():
    ...
    (player_class, player_model_path,
     ...
     show_board) = get_user_inputs()
    
    game, board = init_game(game_type, show_board, max_steps)

    create_player1 = partial(init_player, player_class, player_model_path, game, board, 0, thinking_time)
    create_player2 = partial(init_player, player_class, player_model_path, game, board, 1, thinking_time)
